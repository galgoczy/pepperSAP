...
export default function WorkspacePage() {
  ...
  // Fetch channels and check for unread messages
  ...
  // Check for unread messages in all channels
  const checkUnreadChannels = useCallback(async () => {
    if (!user || channels.length === 0) return;

    // Get user's subscriptions with last_read_at
    const { data: subscriptions } = await supabase
      .from('workspace_subscriptions')
      .select('channel_id, last_read_at')
      .eq('user_id', user.id);

    // Create map of channel_id -> last_read_at
    const lastReadMap = {};
    subscriptions?.forEach(sub => {
      lastReadMap[sub.channel_id] = sub.last_read_at;
    });
    ...
    for (const channel of channels) {
      ...
      if (lastMessage) {
        const lastReadAt = lastReadMap[channel.id];
        // If no subscription or last message is newer than last_read_at
        if (!lastReadAt || new Date(lastMessage.created_at) > new Date(lastReadAt)) {
  ...
  const fetchSubscription = useCallback(async () => {
    ...
    // Update last_read_at and remove from unread set
    if (data) {
      await supabase
        .from('workspace_subscriptions')
    ...
    // Mark channel as read (remove from unread set)
  ...
  // Check unread channels when channels are loaded
  useEffect(() => {
    if (channels.length > 0 && user) {
      checkUnreadChannels();
    }
  }, [channels.length, user, checkUnreadChannels]);
  ...
  // Auto-refresh messages and unread status every 30 seconds
  useEffect(() => {
  ...
  }, [selectedChannel, fetchMessages, channels.length, user, checkUnreadChannels]);
            ...
            {channels.map((channel) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`truncate ${hasUnread ? 'font-bold' : ''}`}>{channel.name}</span>
                  {hasUnread && selectedChannel?.id !== channel.id && (
                    <span className="w-2 h-2 bg-pepper-red rounded-full flex-shrink-0" />
                  )}
                </button>
              );
            })}
